...
termins["глобальное потепление","изменение климата","парниковый эффект","парниковые газы", "атмосферная концентрация","озоновый слой","антропогенные факторы","повышение уровня моря","экстремальные погодные явления","кислотность океана","утрата биоразнообразия","миграция населения","снижение выбрасов","улавление и хранение углерода","адаптация к изменению климата","киотский протокол","парижское соглашения","климатический скептизм","климатический активизм ",""]
...
@bot.command()
async def help(ctx):
    await ctx.send('''В моем боте есть множество команд.Основными командами является: sanitary_waste, battery
